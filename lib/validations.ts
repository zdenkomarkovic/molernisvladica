...
// ─── Kontakt forma (Colorium) ───────────────────────────────────────────────
// Telefon je obavezan (glavni kanal komunikacije), email opcion.

export const molerContactFormSchema = z.object({
  name: z.string().min(2, "Ime mora imati najmanje 2 karaktera").max(100),
  phone: z
    .string()
    .min(6, "Unesite ispravan broj telefona")
    .regex(/^[+]?[\d\s\-().]{6,20}$/, "Unesite ispravan broj telefona"),
  email: z
    .union([z.literal(""), z.string().email("Unesite ispravnu email adresu")])
    .optional(),
  message: z
    .string()
...
export type MolerContactFormData = z.infer<typeof molerContactFormSchema>;
